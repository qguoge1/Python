...
def Generation_trajet () :
    Nb_trajet = int(input("Entrez le nombre de trajets aléatoire à générer :"))
    count = 0
    Nb_Villes = int(input("Entrez le nombre de villes : "))
    Trajet = {}
    while(count< Nb_trajet):            # Crée les variables trajets dans un dictionnaire   Ils seront notés Trajet["Trajet1"], Trajet["Trajet2"]
    ...
    while(count< Nb_trajet):            # Crée les variables trajets dans un dictionnaire   Ils seront notés Trajet["Trajet1"], Trajet["Trajet2"]
        ...
        if(Trajet["Trajet"+str(count+1)].distance < best_distance):
            best_trajet = []
            while(count_v < Nb_Villes):
                best_trajet.append(Trajet["Trajet"+str(count+1)].trajet[count_v].name)
                count_v += 1
            best_distance = Trajet["Trajet"+str(count+1)].distance
        count_v =0
        count += 1
        
    print("Le meilleur trajet est :", best_trajet ,"\n avec une distance de : ", best_distance)
